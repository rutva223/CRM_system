...
@if(isset($meeting))
{{ Form::model($meeting, array('route' => array('meetingUpdate', $deal->id, $meeting->id), 'method' => 'PUT')) }}
@else
{{ Form::open(array('route' => ['meetingStore',$deal->id])) }}
@endif
        ...
        <div class="col-12 form-group">
            ...
            <select name="user_id" id="user_id" class="form-control" required>
                @foreach($users as $user)
                    <option value="{{ $user->getDealUser->id }}" @if(isset($meeting->user_id) && $meeting->user_id == $user->getDealUser->id) selected @endif>{{ $user->getDealUser->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-12 form-group">
